...
module("luci.jsonapi", package.seeall)
require "luci.json"
	...
	if stat then
		if type(json.method) == "string"
		 and (not json.params or type(json.params) == "table") then
			...
			if method then
				response = reply(json.jsonapi, json.id,
...
function reply(jsonapi, id, res, err)
	require "luci.json"
	...
	return {id=id, result=res, error=err, jsonapi=jsonapi}
end
